...
2. **Three consecutive `Pdelay_Req` attempts unanswered.** Mirrors the 802.1AS-2020 §11.2.13 `allowedLostResponses` semantics: the same counter that flips `asCapable` to false at the gPTP layer also triggers fallback at the profile layer. The counter is reset to zero on every received `Pdelay_Resp`. This indicates either an isolated link or a network with no compliant peer.
